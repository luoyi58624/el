...
class MirrorUtils {
    ...
    if (varElement is FieldFormalParameterElement) {
      ...
      if (visitSuper) {
        if (varElement is SuperFormalParameterElement) {
          for (var s in classElement.allSupertypes) {
            final superClass = s.element as ClassElement;
            final field = superClass.getField(varElement.name!);
  ...
  static bool _fieldFilter(FieldElement fieldInfo) {
    return !fieldInfo.isStatic && !fieldInfo.isLate && !fieldInfo.isConst &&
        (fieldInfo.isOriginDeclaration || fieldInfo.isOriginDeclaringFormalParameter);
  }
  ...
  static String generateFieldLerp(FieldElement fieldInfo) {
    ...
    if (fieldInfo.type.isDartCoreDouble) {
      return "$fieldName: lerpDouble(a.$fieldName, b.$fieldName, t) ?? a.$fieldName,";
    } else if (_hasLerp(fieldInfo)) {
      return "$fieldName: $fieldType.lerp(a.$fieldName, b.$fieldName, t) ?? a.$fieldName,";
    } else {
      return "$fieldName: t < 0.5 ? a.$fieldName : b.$fieldName,";
    }
  }
}
...
bool _hasLerp(FieldElement fieldInfo) {
  ...
  return fieldElement is ClassElement && fieldElement.getMethod('lerp') != null;
}
