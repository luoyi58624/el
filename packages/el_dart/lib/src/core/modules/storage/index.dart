...
abstract class ElStorage {
  ...
  ElStorage(this.key, int debounceTime) {
    ...
    execSerialize = El.debounce(write, debounceTime);
  }
  ...
  void setItem<T>(
    ...
    if (expire != null) {
      result = {_expireKey: El.currentMilliseconds + expire.inMilliseconds, _expireDataKey: result};
    }
  ...
  bool checkExpire(String key, {bool includeNull = true}) {
    ...
    if (isExpireData(result)) {
      if (result[_expireKey] <= El.currentMilliseconds) {
        return true;
      }
    }
