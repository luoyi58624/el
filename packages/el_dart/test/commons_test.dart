...
void _safeTest() {
  test('isEmpty: null / 空白字符串 / Iterable / Map', () {
    expect(El.isEmpty(null), true);
    expect(El.isEmpty('   '), true);
    expect(El.isEmpty([]), true);
    expect(El.isEmpty(<int>{}), true);
    expect(El.isEmpty({}), true);
  });

  test("isEmpty: 'null' 仅在 checkString=true 时为空", () {
    expect(El.isEmpty('null'), false);
    expect(El.isEmpty(' null ', checkString: true), true);
    expect(El.isEmpty('NULL', checkString: true), true);
  });

  test('safeString: 空值返回默认值', () {
    expect(El.safeString(null, 'x'), 'x');
    expect(El.safeString('  ', 'x'), 'x');
  });

  test('safeInt: double 不抛异常，字符串支持 trim', () {
    expect(El.safeInt(1.2), 1);
    expect(El.safeInt(' 12 '), 12);
    expect(El.safeInt('1.2', 9), 9);
  });

  test('safeDouble: int 转 double，字符串支持 trim', () {
    expect(El.safeDouble(2), 2.0);
    expect(El.safeDouble(' 1.25 '), 1.25);
    expect(El.safeDouble('x', 9.5), 9.5);
  });

  test('safeBool: 解析失败返回默认值，并支持 0/1', () {
    expect(El.safeBool('TRUE'), true);
    expect(El.safeBool('no', true), true);
    expect(El.safeBool('1'), true);
    expect(El.safeBool('0'), false);
    expect(El.safeBool(1), true);
    expect(El.safeBool(0), false);
  });
}
...
void _compareVersionTest() {
  ...
  test('1.0.0-test < 1.1.0 → 需要更新', () {
    expect(El.compareVersion('1.0.0-test', '1.1.0'), true);
  });

  // 前缀 v / V
  test('v1.0.0 == 1.0.0 → 不更新', () {
    expect(El.compareVersion('v1.0.0', '1.0.0'), false);
  });
  test('V2.1.3 < 2.2.0 → 需要更新', () {
    expect(El.compareVersion('V2.1.3', '2.2.0'), true);
  });

  // 各种后缀
  test('1.0.0+1 == 1.0.0 → 不更新', () {
    expect(El.compareVersion('1.0.0+1', '1.0.0'), false);
  });
  test('1.0.0-beta+hotfix.2 < 1.0.1 → 需要更新', () {
    expect(El.compareVersion('1.0.0-beta+hotfix.2', '1.0.1'), true);
  });
  test('1.0.0-release < 1.0.0 → 不更新', () {
    expect(El.compareVersion('1.0.0-release', '1.0.0'), false);
  });

  // 版本段数不一致
  test('1 < 1.0.0 → 不更新', () {
    expect(El.compareVersion('1', '1.0.0'), false);
  });
  test('1.0 < 1.0.1 → 需要更新', () {
    expect(El.compareVersion('1.0', '1.0.1'), true);
  });
  test('1.0.0.0 < 1.0.0.1 → 需要更新', () {
    expect(El.compareVersion('1.0.0.0', '1.0.0.1'), true);
  });
  test('1.0.0.0.0 == 1 → 不更新', () {
    expect(El.compareVersion('1.0.0.0.0', '1'), false);
  });

  // 跨段大小比较
  test('1.9.9 < 2.0.0 → 需要更新', () {
    expect(El.compareVersion('1.9.9', '2.0.0'), true);
  });
  test('2.0.0 > 1.99.99 → 不更新', () {
    expect(El.compareVersion('2.0.0', '1.99.99'), false);
  });
  test('1.0.10 < 1.0.11 → 需要更新', () {
    expect(El.compareVersion('1.0.10', '1.0.11'), true);
  });
  test('1.0.99 < 1.0.100 → 需要更新', () {
    expect(El.compareVersion('1.0.99', '1.0.100'), true);
  });

  // 异常格式（容错）
  test('空字符串本地 < 1.0.0 → 需要更新', () {
    expect(El.compareVersion('', '1.0.0'), true);
  });
  test('1.0.0 > 空服务端 → 不更新', () {
    expect(El.compareVersion('1.0.0', ''), false);
  });
  test('null 字符串视为 0 → 需要更新', () {
    expect(El.compareVersion('null', '1.0.0'), true);
  });
  test('null 值视为 0 → 需要更新', () {
    expect(El.compareVersion(null, '1.0.0'), true);
  });
  test('服务端 null 值视为 0 → 不更新', () {
    expect(El.compareVersion('1.0.0', null), false);
  });
  test('带空白的 v 前缀版本 == 纯版本 → 不更新', () {
    expect(El.compareVersion('  v1.2.3  ', '  1.2.3 '), false);
  });
  ...
  test('非字符串对象视为 0 → 需要更新', () {
    expect(El.compareVersion({'v': '1.0.0'}, '1.0.0'), true);
  });
  test('版本含字母乱码 < 1.0.0 → 需要更新', () {
    expect(El.compareVersion('a.b.c', '1.0.0'), true);
  });
  test('1.x.3 < 1.2.3 → 需要更新', () {
    expect(El.compareVersion('1.x.3', '1.2.3'), true);
  });

  // 前后都带 v 和后缀
  test('v1.2.3-beta == 1.2.3+1 → 不更新', () {
    expect(El.compareVersion('v1.2.3-beta', '1.2.3+1'), false);
  });
  test('V3.10.9-release < 3.11.0 → 需要更新', () {
    expect(El.compareVersion('V3.10.9-release', '3.11.0'), true);
  });

  // 超大版本号
  test('999.999.999 < 1000.0.0 → 需要更新', () {
    expect(El.compareVersion('999.999.999', '1000.0.0'), true);
  });
}
