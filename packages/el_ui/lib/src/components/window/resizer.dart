...
class _ElWindowResizerState extends State<ElWindowResizer> {
  ...
  void _maximize() {
    ...
    _saveLocalData();
    _maximizeDurationTimer = El.setTimeout(() {
  ...
  void initState() {
    ...
    if (widget.cacheKey != null) {
      ...
      if (localData != null) {
        ...
        _size = Obs(
          Size(El.safeDouble(obj['width'], widget.size.width), El.safeDouble(obj['height'], widget.size.height)),
        );
        _offset = Obs(Offset(El.safeDouble(obj['dx']), El.safeDouble(obj['dy'])));
        _isMaximize = Obs(El.safeBool(obj['isMaximize']));
        flag = false;
      }
    }
