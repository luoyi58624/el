...
class ElButtonGroup extends ElButton {
  /// 普通按钮组
  const ElButtonGroup({
    super.key,
...
class _ElButtonGroupState extends _ElButtonState<ElButtonGroup> {
  late List<Widget> children;
  Map<int, dynamic> itemValueMap = {};

  late final Obs<dynamic> _obs;

  dynamic get modelValue => _obs.value;

  void _onObsChanged() {
    if (widget.modelValue is ValueNotifier) {
      (widget.modelValue as ValueNotifier).value = _obs.value;
    }
    widget.onChanged?.call(_obs.value);
  }

  void _linkRawObs() {
    _obs.rawValue = (widget.modelValue as ValueNotifier).value;
    _obs.notify();
  }
  ...
  void onChanged(dynamic value) {
    switch (widget._modelType) {
      ...
      case ElModelValueType.single:
        if (_obs.value == value && widget.mandatory != true) {
          _obs.value = null;
        } else {
          _obs.value = value;
        }
      ...
      case ElModelValueType.multi:
        assert(_obs.value is List || _obs.value is Set, 'ElButtonGroup.multi 数据类型必须为 List、Set 集合！');
        dynamic list;
        if (_obs.value is List) {
          list = List.from(_obs.value);
        } else {
          list = Set.from(_obs.value);
        }
  ...
  void initState() {
    super.initState();
    if (widget.modelValue is ValueNotifier) {
      final rawObs = widget.modelValue as ValueNotifier;
      _obs = Obs<dynamic>(rawObs.value);
      rawObs.addListener(_linkRawObs);
    } else {
      _obs = Obs<dynamic>(widget.modelValue);
    }
  ...
  void didUpdateWidget(ElButtonGroup oldWidget) {
    ...
    if (widget.modelValue != oldWidget.modelValue) {
      ...
      if (widget.modelValue is ValueNotifier) {
        ...
        safeCallback(() => _obs.value = widget.modelValue);
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Opacity(
      opacity: isInit ? 1 : 0,
      child: ListenableBuilder(listenable: _obs, builder: (context, child) => obsBuilder(context)),
    );
  }
}
