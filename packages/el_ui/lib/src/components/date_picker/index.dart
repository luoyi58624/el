...
class ElDatePicker extends ElModelValue<dynamic> {
  ...
  static ElDatePicker of(BuildContext context) => _ElDatePickerInheritedWidget.of(context);

  /// 构建主题数据
  ElDatePickerThemeData buildThemeData(BuildContext context) {
    return ElDatePickerTheme.of(context).copyWith(primaryColor: primaryColor, format: format);
  }

  /// 显示选择器
  Future<void> showPicker(BuildContext context) async {
    final themeData = buildThemeData(context);

    if (onlyDate != true && onlyTime != true) {
      DateTime? result = await showDatePicker(context, themeData);
      if (result == null) return;

      result = DateTime(result.year, result.month, result.day);

      // ignore: use_build_context_synchronously
      final time = await showTimePicker(context, themeData);
      if (time == null) return;

      result = result.copyWith(hour: time.hour, minute: time.minute);

      if ($obs.value is String) {
        $obs.value = El.formatDate(result, themeData.format ?? 'yyyy-MM-dd HH:mm:ss');
      } else if ($obs.value is int) {
        $obs.value = result.millisecondsSinceEpoch;
      } else if ($obs.value is DateTime) {
        $obs.value = result;
      } else {
      ...
      if (onlyDate == true) {
        final v = await showDatePicker(context, themeData);
        if (v != null) {
          final result = v.copyWith(year: v.year, month: v.month, day: v.day);
          if ($obs.value is String) {
            $obs.value = El.formatDate(result, themeData.format ?? 'yyyy-MM-dd');
          } else if ($obs.value is int) {
            $obs.value = result.millisecondsSinceEpoch;
          } else if ($obs.value is DateTime) {
            $obs.value = result;
        ...
        final v = await showTimePicker(context, themeData);
        if (v != null) {
          if ($obs.value is String) {
            $obs.value = El.formatDate(
              DateTime.now().copyWith(hour: v.hour, minute: v.minute),
              themeData.format ?? 'HH:mm',
            );
          } else if ($obs.value is m.TimeOfDay) {
            $obs.value = v;
          } else {
  ...
  /// 显示日期选择器
  Future<DateTime?> showDatePicker(BuildContext context, ElDatePickerThemeData themeData) async {
    var initialDate = El.safeDate($obs.value);
    var minDate = El.safeDate(firstDate, DateTime(1970));
    var maxDate = El.safeDate(lastDate, DateTime(2050));

    // 对这些时间进行限制，防止 showDatePicker 内部 assert 错误
    if (maxDate.isBefore(minDate)) maxDate = minDate;
    ...
    return await m.showDatePicker(
      ...
      firstDate: minDate,
      lastDate: maxDate,
      builder: (context, child) => buildPickerThemeWidget(context, child!, themeData),
    );
  }

  /// 显示时间选择器
  Future<m.TimeOfDay?> showTimePicker(BuildContext context, ElDatePickerThemeData themeData) async {
    late m.TimeOfDay time;
    try {
      final modelValue = $obs.value;
      if (modelValue is String) {
        if (El.isEmpty(modelValue)) {
          time = m.TimeOfDay.now();
        } else if (modelValue.length <= 5) {
          // 将非数字字符作为分隔符，得到 hour、minute 数组
          final result = modelValue.split(RegExp(r'\D'));
      ...
      assert(false, 'showTimePicker 方法解析 TimeOfDay 失败，当前 modelValue 值为：${$obs.value}');
    }
  ...
  /// 构建选择器主题小部件
  Widget buildPickerThemeWidget(BuildContext context, Widget child, ElDatePickerThemeData themeData) {
  ...
  Widget obsBuilder(BuildContext context) {
    return ElEvent(
      style: ElEventStyle(onTap: (e) => showPicker(context)),
      child: _ElDatePickerInheritedWidget(this, child: child),
    );
  }
}

class _ElDatePickerInheritedWidget extends InheritedWidget {
  const _ElDatePickerInheritedWidget(this.picker, {required super.child});

  final ElDatePicker picker;

  static ElDatePicker of(BuildContext context) {
