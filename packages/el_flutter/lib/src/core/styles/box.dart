...
class ElBoxStyle with EquatableMixin {
  ...
  /// 是否显示盒子，[visible] 为 false 时隐藏但保留占位空间
  final bool? visible;

  /// 背景颜色，[decoration] 的语法糖
  final Color? color;

  /// 边框圆角，[decoration] 的语法糖
  final BorderRadiusGeometry? borderRadius;

  /// 边框样式，[decoration] 的语法糖
  final BoxBorder? border;

  /// 盒子阴影，[decoration] 的语法糖
  final List<BoxShadow>? shadow;

  /// 解析装饰对象，将 [color]、[borderRadius]、[border]、[shadow] 合并到 [decoration] 中
  BoxDecoration? get resolveDecoration {
    if (color == null && borderRadius == null && border == null && shadow == null) {
      return decoration;
    }
    if (decoration != null) {
  ...
  CustomClipper<Path> buildClipper(BuildContext context) {
    final dec = resolveDecoration;
    assert(clipBehavior != null && clipBehavior != Clip.none && dec != null);
    return _DecorationClipper(textDirection: Directionality.maybeOf(context), decoration: dec!);
  }
