...
class GlobalFontUtil {
  ...
  static final Obs<FontModel> _currentFontModel = Obs(GlobalFontPreset.systemFont);

  /// 当前选择的全局字体，null 表示系统字体
  static String? get fontFamily => _currentFontModel.value.fontFamily == '' ? null : _currentFontModel.value.fontFamily;
  ...
  static Future<void> initFont(FontModel fontModel) async {
    ...
    if (initialLocalStr == null) {
      ...
      _initialFont = FontModel.fromJson((jsonDecode(initialLocalStr) as Map).cast<String, dynamic>());
    ...
    try {
      FontModel localFontModel = FontModel.fromJson((jsonDecode(localStr) as Map).cast<String, dynamic>());
      await loadFont(localFontModel);
    } catch (error) {
      ElLog.e(error, title: '本地缓存字体加载异常');
...
Future<bool> _loadFont(String fontFamily, List<ByteData> byteDataList) async {
  ...
  try {
    ...
    ElLog.e(error, title: 'FontLoader加载字体失败');
    return false;
  }
}
