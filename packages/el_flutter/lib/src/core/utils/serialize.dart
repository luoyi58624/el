...
class ElSizeSerialize implements ElSerialize<Size> {
  ...
  Size? deserialize(String? str) {
    ...
    if (map is Map) {
      return Size(El.safeDouble(map['width']), El.safeDouble(map['height']));
    } else {
      return null;
    }
  }
...
class ElOffsetSerialize implements ElSerialize<Offset> {
  ...
  Offset? deserialize(String? str) {
    ...
    final map = jsonDecode(str);
    if (map is Map) {
      return Offset(El.safeDouble(map['dx']), El.safeDouble(map['dy']));
    } else {
      return null;
    }
  }
...
class ElLocaleSerialize implements ElSerialize<Locale> {
  ...
  Locale? deserialize(String? str) {
    ...
    final map = jsonDecode(str);
    if (map is Map) {
      return Locale(El.safeString(map['languageCode']), El.safeString(map['countryCode']));
    } else {
      return null;
    }
  }
