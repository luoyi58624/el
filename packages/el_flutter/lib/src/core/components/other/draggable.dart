...
class ElDraggable<T extends Object> extends LongPressDraggable<T> {
  ...
  DelayedMultiDragGestureRecognizer createRecognizer(GestureMultiDragStartCallback onStart) {
    if (ElPlatform.isDesktop) {
      return ElMultiDragGestureRecognizer(triggerOffset: triggerOffset, allowedButtonsFilter: allowedButtonsFilter)
        ..onStart = onStart;
    } else {
...
class ElMultiDragGestureRecognizer extends DelayedMultiDragGestureRecognizer {
  ...
  MultiDragPointerState createNewPointerState(PointerDownEvent event) {
    if (ElPlatform.isDesktop) {
      return _ElPointerState(event.position, event.kind, gestureSettings, triggerOffset);
    } else {
...
class _ElPointerState extends MultiDragPointerState {
  _ElPointerState(super.initialPosition, super.kind, super.gestureSettings, this.triggerOffset);
  ...
  void checkForResolutionAfterMove() {
    ...
    if (pendingDelta!.dx.abs() > triggerOffset || pendingDelta!.dy.abs() > triggerOffset) {
