...
class TabsData extends InheritedWidget {
  ...
  static TabsData of(BuildContext context) {
    final TabsData? result = context.dependOnInheritedWidgetOfExactType<TabsData>();
...
class GoogleTabsData extends InheritedWidget {
  const GoogleTabsData(this.height, this.radius, {super.key, required super.child});

  final double height;
  final double radius;

  static GoogleTabsData of(BuildContext context) {
    final GoogleTabsData? result = context.dependOnInheritedWidgetOfExactType<GoogleTabsData>();
