...
class ElGoogleTab extends ElBaseTab {
  /// 子标签
  const ElGoogleTab({super.key, required super.title, super.disabled, super.closable});
...
class _TabActiveLayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ColoredBox(
      color: ElBrightness.isDark(context) ? context.elTheme.primary : Colors.white,
  ...
  Widget build(BuildContext context) {
    ...
    return UnconstrainedBox(
      child: SizedBox(
        ...
        child: Padding(
          padding: .symmetric(horizontal: $googleTabData.radius),
          child: Row(
            children: [
              ConstrainedBox(
                constraints: BoxConstraints(maxWidth: $tabsData.childMaxWidth - 80),
                child: ElRichText(
                  title,
                  style: TextStyle(fontSize: $tabsData.fontSize, fontWeight: FontWeight.w500),
                  maxLines: 1,
              ...
              ElEvent(
                style: ElEventStyle(
                  onTap: (e) {
                    final List<ElGoogleTab> tabList = [...$tabsData.children.value].cast();
