...
class _AnimateLoader extends HookWidget {
  const _AnimateLoader({required this.duration, required this.curve});
  ...
  Widget build(BuildContext context) {
    ...
    final AnimationController controller = useAnimationController(duration: duration);
    ...
    double $radius = $data.round
        ? ($data.vertical ? $data.strokeSize.width : $data.strokeSize.height) / 2
        : $data.radius;
