...
enum _ProgressType { line, animate, circle, dashboard }
...
class ElProgress extends StatelessWidget {
  ...
  const ElProgress(
    ...
    this.bgColor,
       ...
       assert(value >= min && value <= max, 'ElProgress value 取值范围必须在 min - max 之间');
  ...
  Widget build(BuildContext context) {
    final $bgColor = bgColor ?? context.elDefaultColor.deepen(5);
    ...
    if (_type == _ProgressType.animate) {
      result = _AnimateLoader(duration: duration, curve: curve);
    } else {
      if (_type == _ProgressType.line) {
        builder = (ratio, strokeSize, color) {
          double $radius = round ? ($vertical ? strokeSize.width : strokeSize.height) / 2 : radius;
          return ClipRRect(
        ...
        builder = (ratio, strokeSize, color) {
          return CustomPaint(
            size: $boxSize,
            painter: _CricleProgressPainter(ratio: ratio, strokeSize: strokeSize, color: color, bgColor: $bgColor),
          );
        };
      } else {
        builder = (ratio, strokeSize, color) {
          return CustomPaint(
            size: $boxSize,
            painter: _DashboardProgressPainter(ratio: ratio, strokeSize: strokeSize, color: color, bgColor: $bgColor),
          );
        };
      }
...
class _ProgressInheritedWidget extends InheritedWidget {
  ...
  static _ProgressInheritedWidget of(BuildContext context) {
    final _ProgressInheritedWidget? result = context.dependOnInheritedWidgetOfExactType<_ProgressInheritedWidget>();
