...
extension ElTreeThemeDataExt on ElTreeThemeData {
  ...
  List<Object?> get _props => [itemHeight, iconSize, parentGap, padding, borderRadius];
}
...
class ElTreeTheme extends StatelessWidget {
  ...
  const ElTreeTheme({super.key, required this.child, required this.data});
  ...
  static ElTreeThemeData of(BuildContext context) =>
      maybeOf(context) ?? (ElBrightness.isDark(context) ? ElTreeThemeData.darkTheme : ElTreeThemeData.theme);

  @override
  Widget build(BuildContext context) {
    final parent = ElTreeTheme.of(context);
    return _ElTreeTheme(data: parent.merge(data), child: child);
  }
}

class _ElTreeTheme extends InheritedWidget {
  const _ElTreeTheme({required super.child, required this.data});
