...
class _ElTableState extends State<ElTable> {
  ...
  Widget buildHorizontalScrollWidget({required Widget child, bool enable = true}) {
    return enable
        ? ScrollConfiguration(
            ...
            behavior: _TableScrollBehavior(enableScrollbar: true, dragScroll: widget.dragScroll),
            child: Scrollbar(
            ...
            Expanded(
              child: _TableColumnScroll(
                ...
                itemBuilder: (context, index) =>
                    _TableRowItem(dataItem: widget.data[index], index: index, columns: columnList),
              ),
            ),
          ],
        ),
      ...
      children: [
        ...
        Expanded(
          child: _TableColumnScroll(
            ...
            itemCount: widget.data.length,
            itemBuilder: (context, index) =>
                _TableRowItem(dataItem: widget.data[index], index: index, columns: fixedLeftColumnList),
          ),
        ),
      ],
    );
      ...
      children: [
        ...
        Expanded(
          child: _TableColumnScroll(
            ...
            itemCount: widget.data.length,
            itemBuilder: (context, index) =>
                _TableRowItem(dataItem: widget.data[index], index: index, columns: fixedRightColumnList),
          ),
        ),
      ],
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(border: Border.all(color: context.elTheme.placeholderTextColor)),
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          Widget child = Row(
            children: [
              ...
              Expanded(
                child: SizedBox(height: constraints.maxHeight, child: buildColumnWidget(columnMaxWidth)),
              ),
              if (fixedRightColumnList.isNotEmpty)
                SizedBox(
...
Widget _buildColumnItemWidget({required Widget child, required ElTableColumn column}) {
  if (column.width != null) {
    ...
    return Expanded(
      child: SizedBox(width: column.minWidth, child: child),
    );
  }
}
...
Border _buildColumnBroder(ElTableColumn column, BorderSide borderSide) {
  ...
  return Border(left: isFixedLeft ? BorderSide.none : borderSide, right: isFixedLeft ? borderSide : BorderSide.none);
}
