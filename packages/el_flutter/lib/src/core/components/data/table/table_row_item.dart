...
class _TableRowItem extends StatefulWidget {
  const _TableRowItem({required this.dataItem, required this.index, required this.columns});
...
class _TableRowItemState extends State<_TableRowItem> {
  ...
  Widget build(BuildContext context) {
    var borderSide = BorderSide(color: context.elTheme.placeholderTextColor);
          ...
          .map(
            (column) => _buildColumnItemWidget(
              child: Container(
                ...
                child: Align(alignment: Alignment.center, child: _buildTableBodyItemContainer(widget.dataItem, column)),
              ),
              column: column,
            ),
          )
        ...
        builder: (context, value, _) {
          return DecoratedBox(
            decoration: BoxDecoration(
              color: elTableData.highlightCurrentRow && value == widget.index ? context.elDefaultColor.deepen(5) : null,
              border: Border(bottom: BorderSide(color: context.elTheme.placeholderTextColor)),
            ),
...
Widget _buildTableBodyItemContainer(Map dataItem, ElTableColumn column) {
  if (column.render != null) {
    ...
    return ElRichText(ElTypeUtil.safeString(dataItem[column.prop] ?? ''));
  } else {
