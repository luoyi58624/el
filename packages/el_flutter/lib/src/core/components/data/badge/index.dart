...
class ElBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (ElDartUtil.isEmpty(badge)) return child;

    final textColor = el.darkTheme.textColor;
    if (badge is num) {
      return Badge(
        label: ConstrainedBox(
          constraints: const BoxConstraints(minWidth: 16),
          child: Center(child: Text(badge > max ? '$max+' : badge.toString())),
        ),
  ...
  TextStyle get textStyle => TextStyle(fontSize: 12, fontWeight: FontWeight.w500);

  /// 让徽章填充样式更加好看，默认情况下徽章给人的感官没有对齐
  EdgeInsetsGeometry get badgePadding {
    if (badge < 10) {
      return const .symmetric(horizontal: 1);
    } else {
      return const .symmetric(horizontal: 4);
    }
  }
}
