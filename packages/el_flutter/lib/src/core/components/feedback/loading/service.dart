...
class ElLoadingService extends ElSingleAnimatedOverlayService {
  ...
  Future<void> open(String text, {ElLoadingCloseModel? closeModel, int? zIndex}) {
    return tasks.run(() async {
      await replace(
        (handle) => _ElLoadingWidget(handle: handle, text: text, closeModel: closeModel, onConfirmClose: close),
        zIndex: zIndex,
      );
    });
  }
...
class _ElLoadingWidget extends ElAnimatedOverlayWidget {
  const _ElLoadingWidget({required super.handle, required this.text, this.closeModel, required this.onConfirmClose});
