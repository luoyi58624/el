...
class ElNullWidget extends Widget {
  ...
  static const ElNullWidget instance = ElNullWidget();
...
class ElNullElement extends Element {
  ElNullElement() : super(ElNullWidget.instance);

  static final ElNullElement instance = ElNullElement();
...
class ElEmptyWidget extends LeafRenderObjectWidget {
  const ElEmptyWidget({super.key});

  static const ElEmptyWidget instance = ElEmptyWidget();
...
class ElFadeTap extends StatelessWidget {
  const ElFadeTap({super.key, this.onTap, this.style, required this.child, this.opacity});
  ...
  @override
  Widget build(BuildContext context) {
    double opacity = this.opacity ?? 0.25;
      ...
      child: Builder(
        builder: (context) {
          return AnimatedOpacity(
            duration: ElPlatform.isDesktop
                ? const Duration(microseconds: 100)
                : (context.hasTap ? Duration.zero : const Duration(milliseconds: 180)),
            curve: Curves.easeOut,
            opacity: context.hasTap ? opacity : 1.0,
            child: child,
          );
        },
      ),
      ...
      child: Builder(
        builder: (context) {
          return AnimatedScale(
            duration: duration,
            curve: Curves.easeOut,
            scale: context.hasTap ? scale : 1.0,
            child: child,
          );
        },
      ),
...
class ElSecondChildElement extends RenderObjectElement {
  ...
  void visitChildren(ElementVisitor visitor) {
    ...
    if (foregroundSecondChildElement != null) {
      visitor(foregroundSecondChildElement!);
    }
  }
...
class ElRenderSecondChildBox extends RenderProxyBox {
  ...
  @override
  void paint(PaintingContext context, Offset offset) {
    if (foregroundSecondChild != null) {
      context.paintChild(foregroundSecondChild!, offset);
    }
  ...
  bool hitTestChildren(BoxHitTestResult result, {required Offset position}) {
    ...
    if (foregroundSecondChild?.hitTest(result, position: position) == true) {
      return true;
    }
    return false;
  }
...
class _LocalResamplingState extends State<_LocalResampling> {
  ...
  Widget build(BuildContext context) {
    ...
    return Listener(
      ...
      onPointerUp: (e) {
        delayCancelTimer = El.setTimeout(() {
          closeResampling();
        }, 3000);
      },
      onPointerCancel: (e) {
