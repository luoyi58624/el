...
class ElNestScrollWrapper extends StatefulWidget {
  /// 优化桌面端嵌套滚动，移动端不会渲染，当鼠标指针滚动内部容器时，
  /// 触顶、触底会添加一个阻尼效果，不会让事件立即扩散到外部滚动容器，
  /// 同时，滚动外部容器时，如果滚动没有停止，那么内部滚动容器不会立即响应事件。
  const ElNestScrollWrapper({super.key, required this.controller, required this.child});

  final ScrollController controller;
  final Widget child;

  @override
  State<ElNestScrollWrapper> createState() => _ElNestScrollWrapperState();
}

class _ElNestScrollWrapperState extends State<ElNestScrollWrapper> {
  ...
  bool get _isScrollTop => widget.controller.position.pixels <= widget.controller.position.minScrollExtent;

  bool get _isScrollBottom => widget.controller.position.pixels >= widget.controller.position.maxScrollExtent;
    ...
    if (e is PointerScrollEvent) {
      ...
      if (_disabledUpScroll) {
        ...
        if (_isScrollTop) {
          ...
          _disabledUpScrollTimer = ElAsyncUtil.setTimeout(() {
            _disabledUpScrollTimer = null;
      ...
      if (_disabledDownScroll) {
        ...
        if (_isScrollBottom) {
          ...
          _disabledDownScrollTimer = ElAsyncUtil.setTimeout(() {
            _disabledDownScrollTimer = null;
  ...
  void _setPreventChild() {
    ...
    if (_preventChildScroll) {
      _preventChildTimer = ElAsyncUtil.setTimeout(() {
        _preventChildTimer = null;
        if (mounted) {
          setState(() {
        ...
        child: Listener(
          ...
          child: NotificationListener<ScrollMetricsNotification>(
            ...
            child: NotificationListener<ScrollUpdateNotification>(
              ...
              child: IgnorePointer(
                ignoring: _NestScrollWrapperInheritedWidget.maybeOf(context)?.preventChildScroll ?? false,
                child: _NestScrollWrapperInheritedWidget(_preventChildScroll, child: widget.child),
              ),
            ),
          ),
        ),
...
class _NestScrollWrapperInheritedWidget extends InheritedWidget {
  const _NestScrollWrapperInheritedWidget(this.preventChildScroll, {required super.child});

  final bool preventChildScroll;

  static _NestScrollWrapperInheritedWidget? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<_NestScrollWrapperInheritedWidget>();
