...
mixin _RawScrollbarMixin<T extends ElScrollbar> on _ElScrollbarMixin<T> {
  ...
  void _handleTrackTapDown(TapDownDetails details) {
    ...
    final thumbRatio = scrollbarPainter._thumbExtent / scrollbarPainter._traversableTrackExtent;
    ...
    switch (axisDirectionToAxis(position.axisDirection)) {
      case Axis.vertical:
        if (details.localPosition.dy > scrollbarPainter._thumbOffset) {
          final tractRatio = details.localPosition.dy / scrollbarPainter._traversableTrackExtent;
          targetOffset = math.min(scrollableExtent * (tractRatio - thumbRatio / 2), position.maxScrollExtent);
        } else {
          final tractRatio = details.localPosition.dy / scrollbarPainter._traversableTrackExtent;
          targetOffset = math.max(scrollableExtent * (tractRatio - thumbRatio / 2), position.minScrollExtent);
        }
      case Axis.horizontal:
        if (details.localPosition.dx > scrollbarPainter._thumbOffset) {
          final tractRatio = details.localPosition.dx / scrollbarPainter._traversableTrackExtent;
          targetOffset = math.min(scrollableExtent * (tractRatio - thumbRatio / 2), position.maxScrollExtent);
        } else {
          final tractRatio = details.localPosition.dx / scrollbarPainter._traversableTrackExtent;
          targetOffset = math.max(scrollableExtent * (tractRatio - thumbRatio / 2), position.minScrollExtent);
    ...
    _cachedController!.animateTo(targetOffset, duration: widget.trackScrollDuration, curve: Curves.easeInOut);
  }
  ...
  bool _canHandleScrollGestures() {
    return widget.ignorePointer != true &&
  ...
  Map<Type, GestureRecognizerFactory> get _gestures {
      ...
      case Axis.horizontal:
        gestures[_HorizontalThumbDragGestureRecognizer] =
            GestureRecognizerFactoryWithHandlers<_HorizontalThumbDragGestureRecognizer>(
              () => _HorizontalThumbDragGestureRecognizer(debugOwner: this, customPaintKey: _scrollbarPainterKey),
      ...
      case Axis.vertical:
        gestures[_VerticalThumbDragGestureRecognizer] =
            GestureRecognizerFactoryWithHandlers<_VerticalThumbDragGestureRecognizer>(
              () => _VerticalThumbDragGestureRecognizer(debugOwner: this, customPaintKey: _scrollbarPainterKey),
              _initThumbDragGestureRecognizer,
            );
      case null:
        return gestures;
    ...
    gestures[_TrackTapGestureRecognizer] = GestureRecognizerFactoryWithHandlers<_TrackTapGestureRecognizer>(
      ...
      (_TrackTapGestureRecognizer instance) {
        if (widget.ignoreTrackPointer != true) {
...
bool _isThumbEvent(GlobalKey customPaintKey, PointerEvent event) {
  ...
  final _ScrollbarPainter painter = customPaint.foregroundPainter! as _ScrollbarPainter;
...
class _VerticalThumbDragGestureRecognizer extends VerticalDragGestureRecognizer {
  _VerticalThumbDragGestureRecognizer({required Object super.debugOwner, required GlobalKey customPaintKey})
    : _customPaintKey = customPaintKey;
...
class _HorizontalThumbDragGestureRecognizer extends HorizontalDragGestureRecognizer {
  _HorizontalThumbDragGestureRecognizer({required Object super.debugOwner, required GlobalKey customPaintKey})
    : _customPaintKey = customPaintKey;
