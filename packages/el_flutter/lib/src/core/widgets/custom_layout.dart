...
class ElCustomMultiChildLayoutWidget extends StatefulWidget {
  /// 简单地扩展自定义多子组件布局，[CustomMultiChildLayout] 只能预先设置父级盒子的尺寸，
  /// 但有时我们希望先测量所有子元素，再决定父级的大小，这就是该组件的作用
  const ElCustomMultiChildLayoutWidget({
    super.key,
  ...
  final MultiChildLayoutDelegate Function(UpdateCustomLayoutSize updateSize, bool isReBuild) delegateBuilder;

  /// 子组件集合，和 [CustomMultiChildLayout] 一样，你必须使用 [LayoutId] 标识它们
  final List<Widget> children;

  @override
  State<ElCustomMultiChildLayoutWidget> createState() => _ElCustomMultiChildLayoutWidgetState();
}

class _ElCustomMultiChildLayoutWidgetState extends State<ElCustomMultiChildLayoutWidget> {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: _size.width,
      height: _size.height,
      child: CustomMultiChildLayout(delegate: widget.delegateBuilder(updateSize, isReBuild), children: widget.children),
    );
  }
}
