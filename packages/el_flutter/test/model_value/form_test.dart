...
void main() {
  ...
  group('ElFormModelValue', () {
    testWidgets('带 prop 的表单项会把 prop 注册到 controller.props', (tester) async {
      final controller = ElFormController(initialValue: <String, dynamic>{'field1': false});
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: ElForm(
              controller: controller,
              child: _TestFormBoolField(prop: 'field1', value: false),
            ),
          ),
        ),
      );
    ...
    testWidgets('validate 会按当前实现清除已注册 prop 对应的错误信息', (tester) async {
      final controller = ElFormController(initialValue: <String, dynamic>{'f': false});
