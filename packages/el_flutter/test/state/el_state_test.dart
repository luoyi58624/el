...
void _baseTest() {
  group('Obs 基础测试', () {
    testWidgets('局部状态测试', (tester) async {
      await tester.pumpWidget(const MaterialApp(home: LocalStateWidget('count')));
      ...
      // 重新挂载小部件，状态将被重置
      await tester.pumpWidget(MaterialApp(home: LocalStateWidget('count')));
        ...
        MaterialApp(
          home: Builder(
            builder: (context) {
              return GestureDetector(
                onTap: () {
                  _push(context, const ChildPage(child: LocalStateWidget('count')));
                },
...
void _memoryLeakTest() {
  group('Obs 内存泄漏测试', () {
    testWidgets('内存泄漏测试1', (tester) async {
      ...
      await tester.pumpWidget(_MainApp(state: state, child: const _NestBuilder()));
      expect(state.activeCountWatch, false);
    ...
    testWidgets('内存泄漏测试2', (tester) async {
      ...
      await tester.pumpWidget(_MainApp(state: state, child: const _StateTestWidget()));
...
class _StateProvider extends InheritedWidget {
  ...
  static _StateProvider of(BuildContext context) {
    final _StateProvider? result = context.dependOnInheritedWidgetOfExactType<_StateProvider>();
