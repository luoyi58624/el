...
void main() {
  ...
  group('ObsBuilder 依赖收集/解绑（复杂场景）', () {
                ...
                children: [
                  ValueListenableBuilder(
                    ...
                    builder: (context, value, _) {
                      return ObsBuilder(builder: (context) => Text(value ? 'A:${a.value}' : 'B:${b.value}'));
                    },
                  ),
                  ElevatedButton(onPressed: () => useA.value = !useA.value, child: const Text('toggle')),
                ],
              );
            },
          ),
    ...
    testWidgets('ignoreObs=true 时读取 Obs.value 不应建立依赖', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(home: ObsBuilder(ignoreObs: true, builder: (context) => Text('v:${obs.value}'))),
      );
        ...
        MaterialApp(
          home: ValueListenableBuilder<bool>(
            ...
            builder: (context, on, _) {
              return Column(
                children: [
                  ElevatedButton(onPressed: () => flag.value = !flag.value, child: const Text('toggle')),
                  ObsBuilder(
                    ignoreObs: true,
